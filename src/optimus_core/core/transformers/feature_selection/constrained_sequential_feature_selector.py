...
class CustomSequentialFeatureSelector(SequentialFeatureSelector, Transformer):
    ...
    def fit(self, X, y):  # pylint:disable=too-many-branches, too-many-ancestors
        """
        ...
        if self.features_to_keep is not None:
            ...
            if set(self.features_to_keep) - set(X.columns):
                raise ValueError(
                    "features_to_keep must be columns in X. "
                    f"Columns {set(self.features_to_keep) - set(X.columns)} not found."
                )
        ...
        for _ in range(n_iterations):
            ...
            new_feature_idx, curr_score = self._get_best_new_feature(
                cloned_estimator, X, y, current_mask, features_to_keep_idx
            )
            ####################################
            ...
            if self.iter_till_improvement:
                if curr_score > global_score_:
                    global_score_ = curr_score
                else:
                    break
            ####################################
    ...
    def _get_best_new_feature(
        self, estimator, X, y, current_mask, features_to_keep_idx
        ...
        return (
            max(scores, key=lambda feature_idx: scores[feature_idx]),
            max(scores.values()),
        )

    def transform(self, x: pd.DataFrame):  # pylint:disable=arguments-differ
