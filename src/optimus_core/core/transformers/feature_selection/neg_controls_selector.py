...
class SimpleNegControlSelector(Transformer):
    ...
    def __init__(
        self,
        estimator: Optional = RandomForestRegressor(),
    ...
    def fit(
        ...
        feat_importances_fte = feat_importances[: -self.n_random_features]  # noqa: E203
    ...
    def create_random_features(self, x: pd.DataFrame):
        """Samples random features and adds them to the input dataframe."""
        n_sample = len(x)

        rnd_ftes = np.random.RandomState(self.random_state).__getattribute__(
            self.noise_distribution
        )(size=(n_sample, self.n_random_features))

        x_new_fte = pd.concat([x, pd.DataFrame(rnd_ftes, index=x.index)], axis=1)
...
class BootstrapNegControlSelector(Transformer):
    ...
    def _run_bootstrap(self, x: pd.DataFrame, y: pd.DataFrame, **fit_params):
        ...
        for _ in range(self.n_bootstrap_samples):
            ...
            feat_importances_fte = feat_importances[
    ...
    def _create_bootstrap_data(self, x: pd.DataFrame, y: pd.DataFrame):
        """Sample from the given dataframe and sample random features."""
        n_sample = len(x)

        # resample features
        sampled_indices = np.random.choice(
            np.arange(x.shape[0]), size=n_sample, replace=True
        )
        x_bootstrap = x.iloc[sampled_indices]
        y_bootstrap = y.iloc[sampled_indices]

        # sample random features
        rnd_ftes = self.rand_num_gen.__getattribute__(self.noise_distribution)(
            size=(n_sample, self.n_random_features)
        )

        x_bootstrap = pd.concat(
            [x_bootstrap, pd.DataFrame(rnd_ftes, index=x_bootstrap.index)], axis=1
        )
...
class NonParametricNegControlSelector(Transformer):
    ...
    @significance.setter
    def significance(self, value: float):
    ...
    def _run_bootstrap(self, x: pd.DataFrame, y: pd.DataFrame, **fit_params):
        ...
        for i in range(self.n_bootstrap_samples):
            ...
            bootstrap_importances.loc[i, :] = _get_feature_importances(model)
    ...
    def _create_bootstrap_data(self, x: pd.DataFrame, y: pd.DataFrame):
        ...
        rnd_fte = self.rand_num_gen.__getattribute__(self.noise_distribution)(
            size=n_sample
        )
...
def _get_feature_importances(estimator):
    ...
    if importances is None and coef_ is not None:
        ...
        raise AttributeError(
            ...
            " or call fit before calling transform." % estimator.__class__.__name__
        )
