...
def calculate_control_sensitivity(
    data: pd.DataFrame, model: SklearnPipeline, params: Dict,
) -> pd.DataFrame:
    """
    ...
    if set(params["control_vars"]) - set(features) != set():
        raise ValueError(
            f"Controls not found in model features:"
            f" {set(params['control_vars']) - set(features)}"
        )

    # calculate sensitivity for each control
    sensitivity = {}
    for var_ in params["control_vars"]:
        pdp, axes = partial_dependence(model, data[features], [var_])

        sensitivity[var_] = {"pdp_min": np.min(pdp), "pdp_max": np.max(pdp)}
    ...
    sensitivity = sensitivity.sort_values("range", ascending=True)
