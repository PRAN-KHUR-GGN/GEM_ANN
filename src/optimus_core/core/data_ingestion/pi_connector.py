...
class BaseOAIConnector(abc.ABC):
    ...
    def plant_data_attrs(self):
        ...
        for dataserver in json_data["Items"]:
            pi_points_url = url + "/" + dataserver["WebId"] + "/points"
            points = requests.get(
                str(pi_points_url),
                auth=(self.pi_api_user, self.pi_api_password),
                verify=self.api_certificate,
    ...
    def _query_tag_webid(self, tag_name):
        ...
        # finalize url endpoint
        url = self.init_url + f"search/query?q=name:{tag_name}"
        response = requests.get(
            str(url),
            auth=(self.pi_api_user, self.pi_api_password),
            verify=self.api_certificate,
        ...
        tag_web_id = pi_api_response["Items"][0]["WebId"]
        return tag_web_id
    ...
    def get_web_id(self, tag_name):
        ...
        if self.plant_data_attrs[self.plant_data_attrs["name"] == tag_name][
            "web_id"
        ].empty:
