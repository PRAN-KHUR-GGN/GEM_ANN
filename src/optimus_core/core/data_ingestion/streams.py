...
from optimus_core.core.data_ingestion.pi_connector import BaseOAIConnector
from optimus_core.core.data_ingestion.utils import get_current_time, convert_timezone


class OAIRecordedStreams(BaseOAIConnector):
    def __init__(
        self, pi_endpoint, pi_api_user, pi_api_password, start_time=None, end_time=None
    ):
        """ Constructor
        Args:
    ...
    def _get_pi_data(self, response_data):
        ...
        for key, value in response_data.items():
            timestamp = []
            data_point = []
            for j_object in value["Content"]["Items"]:
                # collect timestamp
            ...
            # set column names
            sensor_data.columns = ["timestamp", key]
...
class OAISummaryStreams(BaseOAIConnector):
    ...
    def _get_pi_data(self, response_data):
        ...
        Returns:
            dataframe with interpolated sensor data in the correct timezone
        """
        all_tag_values = []
        for key, value in response_data.items():
            timestamps = []
    ...
    def _query_sensor_data(self, tag_list, tag_dict):
        ...
        query_dict["Resource"] = pd.np.where(
            query_dict.data_type == "Digital", interpolate, summary
        )
        # set api call method
        query_dict["Method"] = "GET"
...
class OAICurrentValueStreams(BaseOAIConnector):
    def _get_pi_data(self, response_data):
        ...
        for key, value in response_data.items():
            ...
            current_tag_values[key] = (
                value["Content"]["Value"]["Value"]
                if isinstance(value["Content"]["Value"], dict)
